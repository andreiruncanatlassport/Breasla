  ...
  return (
    <article className="mx-auto max-w-3xl px-5 py-12">
      <Link href="/evenimente" className="inline-flex items-center gap-1.5 text-sm font-medium text-ink-soft hover:text-seal">
        <ArrowLeft className="h-4 w-4" /> Toate evenimentele
      </Link>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-1.5">
        <Badge tone="seal">{TIP_LABEL[eveniment.tip] ?? "Eveniment"}</Badge>
        {eveniment.status === "anulat" && <Badge tone="danger">Anulat</Badge>}
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-1.5">
        <Badge tone="neutral">
          ...
          {new Date(eveniment.data_inceput).toLocaleDateString("ro-RO", { day: "2-digit", month: "long", year: "numeric" })}
          {" · "}
          {new Date(eveniment.data_inceput).toLocaleTimeString("ro-RO", { hour: "2-digit", minute: "2-digit" })}
        </Badge>
        {eveniment.online ? (
          <Badge tone="success">
        ...
        {typeof eveniment.capacitate === "number" && (
          <Badge tone={plin ? "danger" : "warning"}>
            <Users2 className="h-3 w-3" /> {inscrisiCount ?? 0}/{eveniment.capacitate} locuri
          </Badge>
        )}
      </div>
      ...
      {eveniment.link_extern && (
        ...
        >
          Link către eveniment <ExternalLink className="h-3.5 w-3.5" />
        </a>
      )}
