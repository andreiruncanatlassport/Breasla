...
import { UserRound, MessageCircleOff, Trash2 } from "lucide-react";
...
function candDemult(iso: string, locale: string): string {
  const min = Math.floor((Date.now() - new Date(iso).getTime()) / 60000);
  if (min < 1) return locale === "en" ? "now" : "acum";
  if (min < 60) return `${min}m`;
  const ore = Math.floor(min / 60);
  if (ore < 24) return `${ore}h`;
  const zile = Math.floor(ore / 24);
  if (zile < 7) return `${zile}${locale === "en" ? "d" : "z"}`;
  return new Date(iso).toLocaleDateString(locale === "en" ? "en-US" : "ro-RO", { day: "2-digit", month: "2-digit" });
}
...
export function ConversationList() {
  ...
  async function sterge(e: React.MouseEvent, conversationId: string) {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm(t.messages.deleteConfirm)) return;
    setConversatii((prev) => (prev ? prev.filter((c) => c.id !== conversationId) : prev));
    await fetch(`/api/messages/${conversationId}`, { method: "DELETE" });
    if (pathname === `/mesaje/${conversationId}`) router.push("/mesaje");
  }
    ...
    <div className="divide-y divide-line">
      {conversatii.map((c) => (
        <Link
          ...
          href={`/mesaje/${c.id}`}
          className={clsx(
            "group flex items-center gap-3 px-4 py-3.5 transition hover:bg-ink/3",
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="shrink-0 font-mono-num text-[10px] text-ink-soft/60 group-hover:hidden">{candDemult(c.last_message_at, locale)}</span>
            </div>
            <p className={clsx("truncate text-xs", c.necitit ? "font-medium text-ink" : "text-ink-soft")}>
              {c.ultimul_mesaj_eu && (locale === "en" ? "You: " : "Tu: ")}
              {c.ultimul_mesaj ?? "—"}
            </p>
          </div>
          {c.necitit && <span className="h-2 w-2 shrink-0 rounded-full bg-seal group-hover:hidden" />}
          <button
            type="button"
            onClick={(e) => sterge(e, c.id)}
            title={t.messages.delete}
            className="hidden shrink-0 rounded-lg p-1.5 text-ink-soft/60 transition hover:bg-rust/10 hover:text-rust group-hover:block"
          >
