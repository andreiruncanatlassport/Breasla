...
const LABELS_IMPLICITE: OpportunityCardLabels = {
  proiect: "Proiect",
  achizitie: "Achiziție",
...
function formateazaBuget(min: number | null, max: number | null, locale: string): string | null {
...
export function OpportunityCard({
  ...
  const buget = formateazaBuget(opportunity.buget_min, opportunity.buget_max, dateLocale);
    ...
    <Link href={`/oportunitati/${opportunity.id}`} className="group block h-full active:scale-[0.98] transition-transform duration-150">
      <article className="lift-on-hover block-base flex h-full flex-col overflow-hidden p-0">
        ...
        <div className="flex items-start justify-between gap-3">
          <Badge tone={TIP_TONE[opportunity.tip] ?? "neutral"}>{tipLabel}</Badge>
          {opportunity.status === "inchisa" && <Badge tone="danger">{labels.closed}</Badge>}
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-line pt-3">
          ...
          {opportunity.termen_limita && (
            <Badge tone="neutral">
              ...
              {new Date(opportunity.termen_limita).toLocaleDateString(dateLocale, { day: "2-digit", month: "2-digit" })}
            </Badge>
          )}
          {typeof opportunity.raspunsuri_count === "number" && opportunity.raspunsuri_count > 0 && (
            <span className="ml-auto inline-flex items-center gap-1 text-xs font-semibold text-seal">
              {opportunity.raspunsuri_count} {opportunity.raspunsuri_count === 1 ? labels.responseSingular : labels.responsePlural}
